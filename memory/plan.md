...
**Q47 flush math**: Auto_v2 will eventually start each kami once HP hits 95%. First cycle at fresh intensity = ~12-18h until HP enters the danger zone and auto_v2 fires `stop_harvest` (which auto-collects MUSU and credits HARVEST_TIME + scav 1:1). The first kami to stop credits its full cycle's HARVEST_TIME (typically >720min for an 18h cycle). So Q47 should be completable on FIRST observable auto_v2 stop — no force-flush needed. Save the 76M gas a manual start+stop wave would burn. Note: `bountyCollectThreshold` is inert at 10000 — kamis can't accumulate that much before HP forces rest; see strategies/auto_v2-mechanics.md.
...
- **Post-migration HP regen is the rate-limiter.** Session 58/59 misattributed the first-cycle delay to `bountyCollectThreshold` (founder review 2026-04-28: that parameter is inert at 10000 — auto_v2 cycles are HP-driven, `stop_harvest` is what credits MUSU/scav). Session 60 has the right diagnosis: some kamis arrive at the new node with low sync HP (kami 43 at 19%, kami 1064 at low%) from late-cycle stops at the prior node. Auto_v2's 5% safety margin enforces near-full HP before harvest_start, so REST regen must complete first. This adds hours upstream of the cycle (intensity ramp + time-to-health-danger). See [strategies/auto_v2-mechanics.md](../strategies/auto_v2-mechanics.md) for the canonical model.
