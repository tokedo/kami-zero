...
**World view is dynamic — read both signals.** The watcher's `world_targets.json` now scans every node with recent harvest activity (no hardcoded node list). It also surfaces a `hot_battlegrounds` field: nodes where OTHER predators are landing kills in the last 3h, independent of where harvesters happen to be in our scan. At session start, read both `killable_v2` AND `hot_battlegrounds`. A node in `hot_battlegrounds` with no `killable_v2` candidates means another hunter is already extracting from a cluster you can't see clean candidates from — investigate (oracle drill on that node) before pivoting elsewhere. A `killable_v2` row at a node you've never visited is no longer suspicious; it's the watcher doing its job.

**Self-audit via world-wide liquidation feed.** `predator/world-liquidations.jsonl` is an append-only feed of every Discord-announced liquidation in the world (cron `*/10 min`). Each row: `{ts, victim_kami, victim_account, node_name, node_id, attacker_kami, attacker_account, musu, is_self}`. Read the last 3-6 hours at session start. Three uses:

1. **Validate own kills landed** — every `is_self=true` row should match a kill in `predator/metrics.md`. Mismatches = telemetry bug.
2. **Where competitor predators are succeeding** — non-self rows clustered by node_id surface the live "where the action is" map. Cross-reference against `hot_battlegrounds` in `world_targets.json`.
3. **Self-audit missed opportunities** — when a competitor lands kills at a node where your watcher saw above-floor candidates AND you held local instead, that's a doctrine cost. Log to `predator/missed-opportunities.md` (build the file if missing) with the question: *"what doctrine kept me here vs. there, and was it correctly weighted?"* This is your audit, not the founder's. Do it weekly minimum, more often when zero-kill streaks form.
...
**Review the items arsenal every plan.** `predator/items-arsenal.md` is the catalogue of every item with combat/disruption utility — what each does, ingredient cost, and current inventory. At plan time, scan it for items applicable to today's hunt (especially anything that disables enemies, debuffs them, or accelerates our own kamis). When a useful item is missing or low-stock and craftable, add a craft-batch step to the plan. When a useful item exists but is not craftable from current ingredients, propagate to `ideas_to_founder.md` with EV justification.
